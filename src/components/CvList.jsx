...
export default function CvList() {
  ...
  return (
    <section className="flex min-h-screen flex-col items-center bg-[#FAFAFA] px-3 pt-3 pb-12 md:px-4 md:pt-4 md:pb-16">
      <div className="mb-6 text-center md:mb-8">
        ...
        <p className="mx-auto mt-3 max-w-2xl text-sm leading-relaxed text-slate-500 md:text-base">
          Xem xét và lựa chọn những ứng viên phù hợp nhất với nhu cầu tuyển dụng
          của doanh nghiệp.
        </p>
      </div>

      {loading ? (
        <div className="mt-6 text-lg font-medium text-gray-500">
          Đang tải danh sách CV...
        </div>
      ) : cards.length > 0 ? (
        <div className="mt-2 grid w-full max-w-5xl grid-cols-2 gap-2 lg:grid-cols-4">
          {cards.map((cvData) => (
